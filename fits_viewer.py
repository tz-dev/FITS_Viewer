...
Description:
    ...
    font sizes), and navigate through pages. It also includes an Image Viewer for
    displaying image HDUs, with features for zooming, rotating, and navigating through
    images. The tool supports large files via memory mapping (memmap=True) to optimize
    performance.
...
Usage:
    ...
    - Use Mousewheel for page up/down in the table view or image navigation in the Image Viewer.
    - In Image Viewer: Use "Zoom +"/"Zoom -" for scaling, "Rotate Left"/"Rotate Right" for 90-degree rotations,
      and "Previous"/"Next" for image navigation.

Features:
    - Table Viewer: Dynamic column selection, customizable row counts, column widths, and font sizes.
    - Image Viewer: Displays image HDUs with zoom (20% steps), rotation (±90°), and navigation between images.
...
Limitations:
    ...
    - Zoom and rotation in Image Viewer are applied to the displayed image only, not persisted in the FITS file.
"""
